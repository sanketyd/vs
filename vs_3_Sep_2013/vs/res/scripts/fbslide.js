...
function fbslider_album(fb_album_id){
        album_details[fb_album_id]["index"]=(album_details[fb_album_id]["index"]+1)%album_details[fb_album_id]["list"].length;
         target_div.next().text("");
				target_div.next().html("<a target='_blank' title='"+album_details[fb_album_id]["name"]+"' href='http://www.facebook.com/"+fb_album_id+"'>"+album_details[fb_album_id]["name"]+"</a>");
...
$(".FBPage").each(function(){	
	...
	$.getJSON("https://graph.facebook.com/"+$(this).attr('page')+"?fields=albums.limit("+MAX_NUM_ALBUMS+").fields(id,count)", function(data){
		...
		console.log("List of albums to be displayed:");
	...
	$.getJSON(graph,
		(function(FBAlbum) {
			return function(data) {
				...
				$('<div />', {
					class: 'right',
					html: '<a targer="_blank" href="https://www.facebook.com/media/set/?set=a.'+FBAlbum.attr("id")+'">more&raquo;</a>'
				}).insertAfter(FBAlbum);
			}
		})(FBAlbum)
